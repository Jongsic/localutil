...
(function () {
    ...
    // WebKit's PRF support arrived in stages: platform passkeys (iCloud
    // Keychain) since Safari 18, external USB / NFC security keys since Safari
    // 26.2 (WebKit #259934). Two security-key issues remain open as of 2026 —
    // #311099 (hmac-secret handed back still encrypted on some USB / NFC
    // paths) and #314934 (null results from PIN-less UV keys like YubiKey
    // Bio) — and iOS / iPadOS still passes no extension data to external
    // keys. So this is a heads-up, never a gate: whether PRF output actually
    // arrives is decided by the ceremony's own prf.results.first, which
    // prfRegister / prfEvaluate check.
    //
    // severity: 'limited' — external security keys will not return PRF here
    //                       (platform passkeys still work);
    //           'note'    — expected to work, with known rough edges.
    function webkitPrfNote(ua) {
        ua = ua || navigator.userAgent || '';
        const isWebKit = /AppleWebKit/.test(ua) && !/Chrome|Chromium|Edg\//.test(ua);
        if (!isWebKit) return null;
        const isIOS = /iPhone|iPad|iPod/.test(ua) || (/Macintosh/.test(ua) && navigator.maxTouchPoints > 1);
        // Numeric major/minor compare — parseFloat would read 26.10 as 26.1.
        const m = /Version\/(\d+)(?:\.(\d+))?/.exec(ua);
        const version = m ? m[1] + '.' + (m[2] || '0') : null;
        const before262 = m && (+m[1] < 26 || (+m[1] === 26 && +(m[2] || 0) < 2));
        if (isIOS) {
            return {
                engine: 'WebKit', ios: true, severity: 'limited', version,
                reason: 'On iOS / iPadOS, PRF works with platform passkeys (iCloud Keychain), but extension '
                    + 'data still does not reach external USB / NFC security keys — a passkey works here, '
                    + 'a hardware key will come back without PRF output.',
            };
        }
        if (before262) {
            return {
                engine: 'WebKit', ios: false, severity: 'limited', version,
                reason: 'This Safari version delivers PRF only for platform passkeys (iCloud Keychain). '
                    + 'Security-key PRF arrived in Safari 26.2 — update Safari, or use Chrome, Edge or '
                    + 'Firefox with a hardware key.',
            };
        }
        return {
            engine: 'WebKit', ios: false, severity: 'note', version,
            reason: 'Safari supports PRF for platform passkeys and, since 26.2, external security keys — '
                + 'though some keys still hit open WebKit issues (#311099, #314934; e.g. PIN-less keys '
                + 'like YubiKey Bio).',
        };
    }

    async function browserPrfCapability() {
        const out = { webauthn: webauthnAvailable(), reported: null, webkit: webkitPrfNote() };
        try {
    ...
    async function prfEvaluate(opts) {
        ...
        if (!ext || !ext.results || !ext.results.first) {
            // The assertion itself succeeded — this is specifically the browser
            // not delivering prf.results.first, which is the definitive support
            // signal no capability flag can give.
            const wk = webkitPrfNote();
            throw new Error('The key responded, but no PRF output came back with the assertion. '
    ...
    window.LocalUtilKeySource = {
        PBKDF2_ITERS,
        browserPrfCapability, webkitPrfNote,
